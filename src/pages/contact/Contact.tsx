...
const Contact: IPropsJSX  = (props) => {
	...
	const checkInputs = (inputs: NodeListOf<HTMLInputElement>): boolean => {
		...
		Array.from(inputs).forEach((input: HTMLInputElement) => {
			const error: string | boolean = checkInput({text: input.value.trim(), type: input.dataset.type, minLength: Number(input.dataset.min_length), maxLength: Number(input.dataset.max_length)}); 
